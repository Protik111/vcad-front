  ...
  const facts: { icon: InfoIconName; label: string; value: string }[] = [
    {
      icon: "calendar",
      label: "Start Date",
      value: details?.startDate ?? "Contact admissions",
    },
    { icon: "clock", label: "Duration", value: course.duration },
    {
      icon: "briefcase",
      label: "Study Mode",
      value: details?.studyMode ?? "Full Time",
    },
    {
      icon: "pin",
      label: "Locations",
      value: details?.location ?? course.school,
    },
    {
      icon: "tag",
      label: "Tuition Fee (UK)",
      value: details?.tuitionFee ?? "Contact admissions",
    },
    {
      icon: "award",
      label: "Awarding Body",
      value: details?.awardingBody ?? course.school,
    },
  ];
      ...
      <Container className="bg-[url('/images/cta-back-right.png')] bg-top-left bg-no-repeat bg-size-[250px]">
        ...
        <dl className="mt-12 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {facts.map((fact) => (
            <div
              key={fact.label}
              className="flex flex-col items-center gap-3 bg-card px-6 py-9 text-center"
            >
              <span className="flex h-12 w-12 items-center justify-center rounded-full text-white">
                <InfoIcon name={fact.icon} />
              </span>
              <dt className="text-default font-semibold text-white">
                {fact.label}
              </dt>
